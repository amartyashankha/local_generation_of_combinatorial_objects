...
We begin by implementing complex queries such as \func{Next-Neighbor} and \func{Random-Neighbor}
...
\todo[inline,color=red!80!green!25]{We are the first to do \func{Random-Neighbor}}
...
\subsection{Random Graphs (Section~\ref{sec:undirected} and Section~\ref{sec:small_world})}%
...
\textcolor{Maroon}{\func{Next-Neighbor} queries were introduced in \cite{reut}
...
and the sampled color values only depend on common random bits.
