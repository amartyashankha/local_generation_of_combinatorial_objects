    ...
    \begin{algorithmic}[1]
        \Procedure{Accepted}{$v, t$}
            ...
            \For{$w \gets \Gamma(v)$} \label{alg:line:iterate_through_neighbors}
                \Comment{\parbox[t]{.57\linewidth}{Iterate through the neighbors, checking for conflicts}}
                \If {$\vec C_w^t = c$} \label{alg:line:check_immediate_conflict}
                    \Comment{\parbox[t]{.57\linewidth}{Check for conflict with neighbor's current proposal}}
                    \State \Return $\ZERO$
                \EndIf
