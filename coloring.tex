...
\begin{align}
T_{t} &= \textcolor{RawSienna}{\mathlarger\sum\limits_{w\in \Gamma(v)}}\ \
\mathlarger\sum\limits_{t'=1}^{t}\ \  \textcolor{Blue}{\mathbb P[C^{t'}_w = c]}\cdot
\left[ \textcolor{BrickRed}{T_{t'}} + \textcolor{OliveGreen}{W^t_{t'}} \right] \\
&\le \textcolor{RawSienna}{\Delta} \cdot \mathlarger\sum\limits_{t'=1}^{t} \textcolor{Blue}{\mathcal B\left( \frac 1q\right)}\cdot
\left[ \textcolor{BrickRed}{T_{t'}} +
\textcolor{OliveGreen}{T_{t'+1} + \mathcal B\left(\frac{1}{\alpha}\right)\cdot T_{t'+2} + \mathcal B\left(\frac{1}{\alpha^2}\right)\cdot T_{t'+3}
...
Using Lemma~\ref{lem:color_reject_probability} and the fact that $\textcolor{Blue}{\mathbb P[C^{t'}_w = c]}$ is independent of all other events,
