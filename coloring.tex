...
one can use a distributed algorithm for a graph problem to construct a Local Computation Algorithm for the same problem.
Specifically, given a $k$-round distributed algorithm on a network of degree at most $\Delta$,
we can simulate the behaviour of a single node $v$ by simulating the full algorithm on the $k$-neighborhood of $v$.
...
\textcolor{Maroon}{We will show how to reduce the probe complexity by appropriately pruning the $k$-neighborhood.}
...
\begin{framed}
    ...
    \begin{algorithmic}[1]
        \Procedure{Accepted}{$v, t$}
            \State {$c\gets\vec C^t_v$} \Comment{Find the current proposal using the public random bits}
            \For{$w \gets \Gamma(v)$} \Comment{Iterate through the neighbors, checking for conflicts}
                ...
                \Comment{Check for conflict with neighbor's prior state}
                    \If {$\mathcal\vec C^{t'}_w = c$ \textbf{and} \func{Accepted}($w, t'$)} \label{alg:line:check_accepted}
                        ...
                        \State $overwritten\gets \FALSE$ \Comment{Check if color $c$ is overwritten by a future proposal}
...
using only $\mathcal O(n^{3.06/\alpha}\epsilon^{-\mathcal O(1/\alpha)}\Delta\log (n/\epsilon))$ time and random bits.
...
$\mathcal O(n^{3.06/\alpha}\epsilon^{-\mathcal O(1/\alpha)}\Delta\log (n/\epsilon))$ time and random bits, which is sublinear for $\alpha > 3.06$.
\end{proof}
\todo[inline,color=red!80!green!25]{$1/n^c$ close sampling}
