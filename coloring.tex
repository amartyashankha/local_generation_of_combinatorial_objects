...
\begin{framed}
    ...
    \begin{algorithmic}[1]
        \Procedure{Accepted}{$v, t$}
            \State {$c\gets\vec C^t_v$} \Comment{\parbox[t]{.6\linewidth}{\small Find the current proposal using the public random bits}}
            \For{$w \gets \Gamma(v)$} \label{alg:line:iterate_through_neighbors}
                ...
                \If {$\vec C_w^t = c$} \label{alg:line:check_immediate_conflict}
                ...
                \Comment{\parbox[t]{.45\linewidth}{\footnotesize Check for conflict with neighbor's prior state}}
                    \If {$\mathcal\vec C^{t'}_w = c$ \textbf{and} \func{Accepted}($w, t'$)} \label{alg:line:check_accepted}
                        \Comment{\parbox[t]{.45\linewidth}{\footnotesize Potential conflict with neighbor's color}}
                        \State $overwritten\gets \FALSE$ \Comment{\parbox[t]{.45\linewidth}{\footnotesize Check if color $c$ is overwritten by a future proposal}}
