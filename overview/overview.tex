...
\end{definition}
For example, the \emph{family} of Erdos-Renyi graphs maps $\Pi = (n, p)$ to a distribution over all $n$-vertex graphs,
where the probability assosiated with any graph containing $m$ edges is exactly $p^m\cdot (1-p)^{\binom{n}{2}-m}$.
...
Given the description $\Pi$ and a query $F_i$, the oracle returns the value $\mathcal M(\Pi, F_i)$,
