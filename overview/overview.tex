...
In this setting, the oracle from Definition~\ref{def:local_access} cannot read all of $\Pi$, and instead accesses it through local probes.
For instance, consider the \textbf{random object family} that maps a graph $G$ to the uniform distribution over all valid colorings of $G$.
Here, the parameter $\Pi$ includes the entire underlying graph $G$, which is too large to be read by a sublinear time algorithm.
