...
When only given access to queries to the probability distribution function of $\{p_i\}$, sampling can be more challenging.
...
the following Lemma from \cite{huge} formalizes this concept. %, and shows that if we can provide such a $\{ q_i\}$,
%we can quickly sample according to a distribution that is close to $\{ p_i\}$.
\begin{lemma}
