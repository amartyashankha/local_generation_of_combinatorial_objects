...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Procedure{Split}{$U, D, k$}
            \State{$B \gets U + D$}\\
            \State{$d \sim \left\{\frac{\binom{B}{B-d}\binom{B}{D+d}}{\binom{2B}{2D}}\right\}_d$}\\\\
            \State{$k' \gets k + U - D$}\\
            \State{$p_d \gets
                \frac{{{B}\choose{D-d}}-{{B}\choose{D-d-k}}{{B}\choose{U-d}}
                -{{B}\choose{U-d-k'}}}{{{2B}\choose{2D}}-{{2B}\choose{2D-k}}}$}
            \State{$q_d \gets \frac{\binom{B}{B-d}\binom{B}{D+d}}{\binom{2B}{2D}}$}\\
            \If {$p_d < q_d$}
                \State \Return $d$
            \EndIf
