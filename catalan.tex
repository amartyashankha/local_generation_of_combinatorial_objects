...
We will use the rejection sampling lemma (Lemma~\ref{lem:rejection_sampling}, restated from \cite{huge}).
...
An important caveat here is that we do not consider first returns in the case that the step from $x$ to $x+1$ is an down-step.
Of course, we could solve the same problem by flipping it upside down,
but the aforementioned correspondence to other Catalan objects only requires the ability to sample first returns from ``above''
(see Figure~\ref{fig:dyck_tree_bijection}).
